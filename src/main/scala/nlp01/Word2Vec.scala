...
object RunWord2Vec {
  ...
  def main(args: Array[String]) {
    ...
    val model = new Word2Vec()
    model.load("/home/robson/Projects/dsa-nlp/src/main/scala/nlp01/vectors.bin")

    // distance: Find N closest words
    println("\nEncontrando as palavras mais próximas a France e USA:")
    ...
    println("\nSe a palavra King é próxima a palavra Queen, a palavra Man é próxima a:")
    model.pprint(model.analogy("king", "queen", "man", N = 10))

    // rank: Rank a set of words by their respective distance to the central term
    println("\nRank das palavras por suas respectivas distâncias ao termo central Apple:")
